export class PicReturn {
  ...
  constructor(private picBox: any, private timeOut: number = null, private points: any = null,
              private pointSelectStyle: string = null) {
    this.pictureBox = this.picBox.nativeElement;
    this.allPicture = [];
    this.allPoint = [];
    this.allPictureNormal = [];
    this.currentIndex = 0;
    if (points !== null) {
      this.pointsBox = this.points.nativeElement;
    } else {
      this.pointsBox = null;
    }
  }
  ...
  loopPoint() {
    if (this.pointsBox !== null) {
      ...
      const attrName: string = this.pointsBox.attributes[0].name;
      for (let i = 0; i < this.allPicture.length - 1; i++) {
        ...
        onePoint.setAttribute(attrName, '');  // ie9
